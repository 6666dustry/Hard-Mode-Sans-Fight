export default function update(draws, updateDisplay) {
    if (updateDisplay) {
        ...
        if (this.moving) {
            const THICKNESS_DOUBLE = this.thickness * 2;
            const THICKNESS_HALF = this.thickness * 0.5;
            this.up.setDisplaySize(this.RectSize.width + THICKNESS_DOUBLE, this.thickness);
            this.up.setPosition(this.RectSize.centerX, this.RectSize.top - THICKNESS_HALF);
            this.down.setDisplaySize(this.RectSize.width + THICKNESS_DOUBLE, this.thickness);
            this.down.setPosition(this.RectSize.centerX, this.RectSize.bottom + THICKNESS_HALF);
            this.left.setDisplaySize(this.thickness, this.RectSize.height + THICKNESS_DOUBLE);
            this.left.setPosition(this.RectSize.left - THICKNESS_HALF, this.RectSize.centerY);
            this.right.setDisplaySize(this.thickness, this.RectSize.height + THICKNESS_DOUBLE);
            this.right.setPosition(this.RectSize.right + THICKNESS_HALF, this.RectSize.centerY);
            if (this.RectBox) {
                this.RectBox.clear();
