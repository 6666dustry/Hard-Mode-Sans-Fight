...
function setLength() {
    ...
    this.scene.matter.body.scale(BODY, this.platLength / this.oldLength, 1);
...
export default function update() {
    ...
    let angle = Phaser.Math.Angle.BetweenPointsY({ x: this.x, y: this.y }, edge);
    /**radians */
    let nowRot = -this.rotation;
    let left = nowRot - Math.PI / 2;
    let right = nowRot + Math.PI / 2;
    let canRide = false;
    ...
    if (canRide) {
        //friction system.
        this.scene.matter.overlap(this.body, [this.scene.director.Heart.Image.body], () => {
            if (this.color === 0) {
